...
Hydratable SSR also trims metadata where possible: static or anchor-only roots can omit a root `nodeRef` entirely, and roots that only need structural claim do not keep a `data-f-node` marker in the HTML payload.
